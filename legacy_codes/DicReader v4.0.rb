...
def m_explains
  if @ln == 26
    print "\033[F\033[s\n"
    @wd[1].each {|x| print "\n "; print "\033[1;4;7;40;37m#{x[1]}\033[0m	\033[1;41;37m例句:\033[0m #{x[0].gsub(/(?<foo>\b#{@wd[0].gsub(/(?<foo>.)(\W+)/, '(\k<foo>|.s|.es|.{0,2}ed|.{0,2}ing)\b|\b')}\b)/i, "\033[7m\\k<foo>\033[0m").gsub("\033[0m \033[7m", ' ').split(':', 2)[1]}\n"}
    puts; puts
  else
...
loop do
  ...
  while @wn < @hash.size
    ...
    if r == "\n"
      ...
      puts; print "\033[s"
      if @ln == 26
        @wd[1].each {|e| puts " \033[1;41;37m例句\033[0m	#{e[0].gsub(/(?<foo>\b#{@wd[0].gsub(/(?<foo>.)(\W+)/, '(\k<foo>|.s|.es|.{0,2}ed|.{0,2}ing)\b|\b')}\b)/i, "\033[7m\\k<foo>\033[0m").gsub("\033[0m \033[7m", ' ').split(':', 2)[1]}"; puts}
        print ' '; system('pause')
      else
        if @exmp[@wd[0]].empty? then print "\033[u 暂时没有可用或可靠的网络例句. "; system('pause') end
        @exmp[@wd[0]].each_with_index do |x, i|
  ...
  loop do
    ...
    if @rec[1..-1].include?(@rn)
      ...
      while @wn < @hash_del[@rn].size
        ...
        @wd = @list[@wnl[@wn - 1]]
        print " \033[1;4;7;44;37m[#{@wnl[@wn - 1] + 1}]\033[0m "; print @wd[0]; print '	'; r = gets.chomp
        if r.empty?
        elsif r.ord > 47 and r.ord < 58
          if @wn > 1
            print "\033[u"; sleep(0.05); print "\033[J"; sleep(0.05)
