...
loop do
  ...
  while @input.size < @result.size
    ...
    if lin == 25
      @wd[1].each {|x| print "\n "; print "\033[1;4;7;40;37m#{x[1].split('||')[0]}\033[0m	\033[1;41;37m例句:\033[0m #{x[0].gsub(/(?<foo>\b#{@wd[0].gsub(/(?<foo>.)(\W+)/, '(\k<foo>|.s|.es|.{0,2}ed|.{0,2}ing)\b|\b')}\b)/i, "\033[7m\\k<foo>\033[0m").gsub("\033[0m \033[7m", ' ').split(':', 2)[1]}\n"}
      puts; puts
    else
